...
import { formatMoneyByTenMillon, decimalFormat, formatMoneyByTrdDecimal, formatByTrd, formatMoneyByTrd } from 'tool/format'
        ...
        <Tooltip overlayClassName={styles.tooltipDesc} arrow={false} placement="bottomRight" title={renderContent}>
          {/* <div onClick={handleDetail} style={{ cursor: 'pointer' }}> */}
            ...
            <Progress
              ...
              // @ts-ignore
              percent={standardValue && standardValue !== 0 ? decimalFormat(Math.abs(value / standardValue) * 100) : '0.00'}
              format={percent => (nameFailed == 'UV' || nameFailed== 'SKU数量') ?formatMoneyByTrd(Number(value)): unit ? `${formatByTrd(Number(value))} ${unit}`:formatByTrd(Number(value))}
              size="small"
            />
          </div>
          {/* @ts-ignore */}
          {/* {max && min && (
            <div className={styles.warpd}>
              <div className={styles.rangeWrap} style={{ left: `${left * 100}%`, width: `calc(${width * 100}%)` }}>
  ...
  if ([CategoryItemType.process].includes(type)) {
    return (
      ...
      >
        {detailData ? (
          <Tooltip overlayClassName={styles.tooltipDesc} arrow={false} placement="bottomRight" title={renderContent}>
            {/* <div onClick={handleDetail} style={{ cursor: 'pointer', position: 'relative', top: '-1px' }}> */}
              ...
              <Progress
                ...
                // @ts-ignore
                percent={standardValue && standardValue !== 0 ? decimalFormat((Math.abs(value / standardValue)) * 100) : '0.00'}
                format={percent => nameFailed == 'UV' ? formatMoneyByTrd(Number(value)): unit ? `${formatByTrd(Number(value))} ${unit}`:formatByTrd(Number(value))}
                size="small"
              />
            </div>
          </Tooltip>
