...
const CategoriesDashboard = (props: any) => {
  ...
  const queryTraceData = async (param: any) => {
    const res = await getTrace(param)
    if (res && res.data && res.data.code === 200 && res.data.data) {
      setTraceData(res.data.data)
      // setIsEmpaty(res.data.data)
    }
  }

  // 获取大事件
  const queryBigEvent = async (param: any) => {
    const res = await getBigEvent(param)
    if (res && res.data && res.data.code === 200 && res.data.data) {
  ...
  const queryBoundary = async (param: any) => {
    const res = await getBoundary(param)
    if (res && res.data && res.data.code === 200 && res.data.data) {
  ...
  const items: TabsProps['items'] = [
    {
      key: '大事件管理',
      label: `大事件管理`,
      children: <BigEventManage formData={formData} bigEventData={bigEventData} boundaryData={boundaryData} level={0} />,
    },
    {
      key: '目标追踪',
      label: `目标追踪`,
      ...
      {!isEmpty ? (
        <>
          <div className={styles.topWrap}>
            ...
            <div>
              <TrackingTable list={traceData} level={0} />
            </div>
          </div>
